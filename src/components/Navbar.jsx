  ...
  return (
    <header className="sticky top-0 z-40 border-b border-gray-200 bg-white/95 backdrop-blur">
      <div className="w-full px-[5%]">
        ...
        {mobileOpen && (
          <div className="xl:hidden border-t border-gray-200 py-4 space-y-3">
            {isAuthenticated && (
              <div className="px-3 py-2 rounded-md bg-gray-50 border border-gray-200 flex items-center justify-between">
            ...
            <nav className="grid grid-cols-1 gap-1" aria-label="Mobile Primary Navigation">
              {primaryLinks.map((link) => (
                <NavLink
                  key={link.to}
                  to={link.to}
            ...
            {isAuthenticated ? (
              <div className="grid grid-cols-2 gap-2 pt-2">
                <Link to="/volunteer/dashboard" onClick={() => setMobileOpen(false)} className="fluent-btn fluent-btn-ghost justify-center">Dashboard</Link>
                {hasVolunteered && (
                  <Link to="/get-involved" onClick={() => setMobileOpen(false)} className="fluent-btn fluent-btn-ghost justify-center">{t('nav.getInvolved')}</Link>
                )}
                <Link to="/tasks" onClick={() => setMobileOpen(false)} className="fluent-btn fluent-btn-ghost justify-center">Tasks</Link>
                <Link to="/leaderboard" onClick={() => setMobileOpen(false)} className="fluent-btn fluent-btn-ghost justify-center">Leaderboard</Link>
                <Link to="/profile" onClick={() => setMobileOpen(false)} className="fluent-btn fluent-btn-ghost justify-center">Profile</Link>
                <Link to="/notification-settings" onClick={() => setMobileOpen(false)} className="fluent-btn fluent-btn-ghost justify-center">Notifications</Link>
                {user?.roles?.includes('Admin') && (
                  <Link to="/admin" onClick={() => setMobileOpen(false)} className="fluent-btn fluent-btn-ghost justify-center text-purple-600">Admin</Link>
                )}
                <button onClick={handleLogout} className="fluent-btn fluent-btn-ghost justify-center text-red-600">{t('auth.logout') || 'Logout'}</button>
              </div>
            ) : (
              <div className="grid grid-cols-2 gap-2 pt-2">
